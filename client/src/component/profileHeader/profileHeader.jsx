...
import { Box, Avatar, Typography, Stack, Button, Tabs, Tab, Divider, FormControlLabel, Checkbox } from '@mui/material'

import React, { useEffect, useState } from 'react'
import { useSelector } from "react-redux"
import { editUser, followUnfollowUser } from "../../feature/userSlice.js"
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-start", width: "100%" }} px={4} py={2} >
            <Stack width={"60%"} pl={3} gap={1} >
                <Stack flexDirection={"row"} alignItems={"center"} gap={5} >
                    ...
                    <Stack>
                        <Typography variant="h4" color="black" fontWeight={"600"}>{user?.userName}</Typography>
                        {/* { } */}
                        ...
                        {!isUserItself && <Button disableElevation disableTouchRipple variant='contained' sx={{ background: "#da254b", "&:hover": { bgcolor: "#df4263" }, mt: 2 }} onClick={handleClick}>
                            {doesListen ? "Tune out" : "Listen"}
                        </Button>}
                        {isUserItself && <>
                            <Button disableTouchRipple disableElevation variant='contained' sx={{ background: "#da254b", "&:hover": { background: "#df4263" }, mt: 2 }} onClick={() => setOpen(true)}>Edit Profile</Button>
                ...
                <StyledBox sx={{ display: "flex", alignItems: "center", justifyContent: "space-around" }} >
                    <Typography variant="body1" color="black"> Listener <span style={{ fontWeight: 800 }}>{listnerNumber}</span></Typography>
                    <Typography variant="body1" color="black"> Listening  <span style={{ fontWeight: 800 }}>{user?.lisetning?.length} </span></Typography>

                </StyledBox>
                {
                    user?.bio && user?.bio.split("\n").map(singleBio => {
                        return <Typography fontWeight={100} variant="body1" color="initial">{singleBio}</Typography>
                    })
                }
                {
                    isUserItself &&
